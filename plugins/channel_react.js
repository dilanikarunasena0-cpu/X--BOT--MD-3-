    ...
    async function(conn, m, { text }) {
        try {
            ...
            if (!channelLink || !channelLink.includes('whatsapp.com/channel/')) {
                return m.reply('❌ කරුණාකර වලංගු WhatsApp Channel සබැඳියක් ඇතුළත් කරන්න.');
            }

            let count = parseInt(countInput);
            if (isNaN(count) || count <= 0) count = 1;
            if (count > 30) count = 30; // ආරක්ෂාව සඳහා සීමා කිරීම

            let delaySec = parseFloat(delayInput);
            if (isNaN(delaySec) || delaySec < 0.5) delaySec = 0.5;

            m.reply('🔄 නාලිකා දත්ත පරීක්ෂා කරමින් පවතිනවා. කරුණාකර රැඳී සිටින්න...');
            ...
            return m.reply(`❌ පද්ධතිමය දෝෂයක් සිදු විය: ${globalError.message}`);
        }
    }
};
