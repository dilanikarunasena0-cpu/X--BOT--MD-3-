...
Sparky({
    ...
    try {
        ...
        // Loader එකක් යැවීම
        await client.sendMessage(m.jid, { text: "📥 NewsAPI සේවාදායකයෙන් ලංකාවේ අලුත්ම පුවත් ලබාගනිමින් පවතී..." }, { quoted: m });

        // country=lk මඟින් ශ්‍රී ලංකාවේ පුවත් පමණක් ලබා ගනී
        const apiKey = "15d4000cd98b4ec59387a8bbb1bb5372";
        const response = await axios.get(`https://newsapi.org/v2/top-headlines?country=lk&apiKey=${apiKey}`, {
            timeout: 10000
        });
        ...
        if (!articles || articles.length === 0) {
            return await m.reply("❌ මේ වෙලාවේ ලංකාවේ කිසිදු අලුත් පුවතක් සොයාගත නොහැකි විය.");
        }
...
│ *📰 ${botName} NEWS HUB*
├───────────────◉
│✨ NewsAPI Official Service (Sri Lanka)
├───────────────◉\n`;
...
`;
        ...
        // ඕනෑම ආකාරයක Text එකක් නිවැරදිව කියවා ගන්නා Function එකක්
        ...
        // වඩාත් නිවැරදිව Reply එක අංකයක් ලෙස ලබා ගැනීම
        const userReplyText = getMessageText(replyMsg);
        const selectedIndex = parseInt(userReplyText) - 1;

        if (isNaN(selectedIndex) || selectedIndex < 0 || selectedIndex >= topNews.length) return;
...
📌 *විස්තරය:* ${selectedNews.description || selectedNews.content || "සම්පූර්ණ විස්තරය ලබාගත නොහැක."}
...
`;
        ...
        // පුවතේ පින්තූරයක් තිබේ නම් එය සමඟ විස්තරය යැවීම
